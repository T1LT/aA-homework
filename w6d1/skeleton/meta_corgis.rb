...
class MetaCorgiSnacks
  ...
  #   # Your code goes here...
  #   info_sender = "get_#{name.to_s}_info".to_sym
  #   taste_sender = "get_#{name.to_s}_tastiness".to_sym
  #   info = @snack_box.send(info_sender, @box_id)
  #   tastiness = @snack_box.send(taste_sender, @box_id)
  #   result = "Treat: #{info}: #{tastiness} "
  #   tastiness > 30 ? "* #{result}" : result
  # end
